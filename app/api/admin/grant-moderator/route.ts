...
export async function POST(req: NextRequest) {
  try {
    ...
    if (profileId === meId) {
      return NextResponse.json({ success: false, error: "Du hast bereits Moderator-Rechte." }, { status: 400 })
    }
    ...
    if (target.role === "moderator") {
      return NextResponse.json({ success: true, alreadyModerator: true })
    }
    ...
    const message = error instanceof Error ? error.message : "Grant moderator failed"
