...
export async function POST(req: NextRequest) {
  try {
    ...
    if (profileId === meId) {
      return NextResponse.json({ success: false, error: "Du kannst dir die Admin-Rolle nicht selbst entfernen." }, { status: 400 })
    }
    ...
    const message = error instanceof Error ? error.message : "Remove admin failed"
